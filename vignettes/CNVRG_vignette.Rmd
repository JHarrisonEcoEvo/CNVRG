...
If you are new to statistical modelling and want a brief primer that explains the rationale behind the use of the Dirichlet and multinomial distributions for count data see this video that I made: https://use.vg/OSVhFJ

In this vignette, we demonstrate how to use 'CNVRG' on a simple dataset. We urge new users to carefully examine the files used. Like any software, data must be formatted properly for 'CNVRG' to work and if you have problems check input format first!

We also recommend that application of 'CNVRG' to larger data (e.g., a matrix of several thousand by several hundred) be performed remotely as modelling can take some compute time. As a head's up to new users, it is worth subsetting one's data and ensuring code correctness and model completion when using the smaller, subsetted data prior to analyzing a large dataset.
...
If you have zeros in your data (like we all do) then these can cause problems because the log of zero is not defined. Therefore, you should add a constant to all your data (i.e., 1). If you are concerned that this could cause problems, then perhaps try running the analysis with a few constants and see if your inference is affected. How to deal with zeros in community ecology data is a topic of research, if you have suggestions let me know.
```{r}
fungi[1:3,1:5]
```
...
If model diagnostics seem sufficient then further analyses can be performed. Use the 'extract' function from 'RStan' to assign samples for parameters of interest to an object. These samples can then be passed to downstream analyses.
...
This function subtracts the posterior distribution of the pi parameter for each feature in one treatment group from the pi parameter distribution in other treatment groups. The function outputs a matrix of proportions that describes the proportion of the distribution of differences that is greater than zero, for each comparison. In this example, the comparison between treatment 1 and treatment 3 provided the following results.

```{r}
diff_abund_test[[1]][,1:2]
```

This means that for Otu 4 nearly 100% of the samples obtained after subtracting the pi distributions for that Otu from treatment 1 from treatment 2 were greater than zero (and conversely when subtracting treatment 2 from 1 zero of them were greater than zero). This means that the there is high certainty that the pi value for Otu 4 in treatment 1 was greater than in treatment 2. Stated another way, this means that the relative abundance of Otu 4 was greater in treatment 1 than in treatment 2.

```{r}
diff_abund_test[[1]][,c(1,3)]
```

The same was true for Otu 58. However, if we compare treatment groups 1 and 3 for this OTU we see that somewhere between a quarter and a third of samples were above zero in the distribution of differences. So we have much less certainty that this Otu differed in relative abundance between treatment group 1 and treatment group 3.

Because the 'diff_abund' function provides insight into the proportion of samples ABOVE zero, values that are very large and very small (e.g., >0.95 or <0.05) denote a high certainty of an effect of treatment group (since we also are interested in cases when few samples are above zero).
...
```{r}
transformed_data <- isd_transform(model_out = modelOut, countData = fungi, isd_index = 2)
```

It is worth checking that the correct index for the ISD was provided. Examine the output and make sure that the field for the index is filled with ones and that field is indeed the ISD.

Transformed data can be used in the 'diversity_calc', 'diff_abund', and 'extract_point_estimates' functions.

A final note: CNVRG is not my focus as a researcher, still I will support this software for the time being. I may post a few updates here and there and I can respond to questions, just email me. If you use this software consider citing it and the associated paper (2020a below). Cheers!
